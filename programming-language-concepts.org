...
this file was messy, just org'd it, it's very incomplete..

probably can merge from [haxe] and [ruby]
...
these notes generally use haxe and ruby as examples
  - which is good, as it covers an advanced static lang and a complex dynamic lang
...
**** block/lambda/proc/closure (ruby)
***** blocks
...
generally the answers are a Main class for input, and a target platform: this could be source code, bytecode, machine code
...
** standard library
although not part of the actual programming language, it's bundled along with most general programming languages (embedded languages are an exception)

*** standard library structure

std lib structure from haxe's docs, expanded a few categories:
  - this is pretty much the same in most languages (as haxe transpiles to so many, it is quite generalized)
...
TODO: there's also target-specific stuff..
