...
systems langs () -> target platform assembly
  - C, C++, rust, zig, go, etc.
    - gcc, msvc, cargo/zig/go build, etc.

high-level lang -> llvm ir -> target platform assembly
  - (each lang provides it's own build cli tools/abstraction over llvm)

high-level lang -> generated C (to link to consoles, mobile) (-> LLVM for optimizing) -> target platform assembly
...
high-level lang -> intermediate representation (ir, byte-code) -> VM (-> machine code?)
  - can vary from a simple line-by-line interpreter or JIT with cached optimizations
  - byte-code (and all ir?) may be pre-compiled
  - byte-code is a specific type of ir
  - java (1995), c# (2002), haxe (2005 neko, 2017 hashlink), ruby (2007 yarv, 2018 yjit), python (2024?), elixir, julia, etc.
  - embedded langs

high-level lang -> interpreter
  - direct from high-level to machine code..?
    - ai said nahhhh son
...
WARNING: for iOS and all game consoles, "cannot execute writeable regions", thus cannot run a system-level VM (like JVM), though, it can run embeddable non-JIT VMs (lua, without JIT)
